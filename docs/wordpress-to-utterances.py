...
def issues_by_post(title, link_name, url):
    """
    ...
    filtered = [i for i in issues if i.title == link_name]
    if filtered:
        return filtered[0]        
    
    issue = repo.create_issue(title=link_name, body=f"#{name}\n\n\n\n[{url}]({url})")
    time.sleep(5)
    issues.append(issue)
    return issue
...
for cmnt in sorted(map(xml_wrapper, dom.getElementsByTagName('wp:comment')), key=lambda c: int(c.comment_id)):
    ...
    issue = issues_by_post(cmnt.parent.title, link_name, url=cmnt.parent.link)
